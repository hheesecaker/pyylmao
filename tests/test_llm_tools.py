...
class LLMToolRegistryTests(unittest.TestCase):
    ...
    def test_read_command_accepts_logged_alias_names(self) -> None:
        ...
        self.assertIn("def parse_prompt", registry.execute(context, "read_command", {"name": "gpt.common"}))
        self.assertIn("def execute_tool", registry.execute(context, "read_command", {"name": "gpt.tools"}))
        self.assertIn("TOTAL_COST_KEY", registry.execute(context, "read_command", {"name": "gpt.billing"}))
        self.assertIn("class MDBuffer", registry.execute(context, "read_command", {"name": "mdbuffer"}))
